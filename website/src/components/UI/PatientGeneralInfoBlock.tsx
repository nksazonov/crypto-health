...
interface Props {
  patient: PatientGeneralInfo | null,
  onEditClick?: () => void,
  onDeleteClick?: () => void,
  className?: string,
}

function PatientGeneralInfoBlock({ patient, onEditClick, onDeleteClick, className }: Props) {
  return (
    <div className={`bg-blue-light px-14 py-10 ${className || ''}`}>
      ...
      { patient !== null
        ...
        <>
          ...
          {onDeleteClick && <div className="flex justify-center mt-16"><Button text="Delete patient" onClick={onDeleteClick} negative /></div>}
        </>
        :
        <div className="text-2xl text-center">No information available!</div>
        }
    </div>
  );
}
