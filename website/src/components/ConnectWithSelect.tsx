...
export function ConnectWithSelect({
  connector,
  activeChainId,
  chainIds = Object.keys(CHAINS).map(Number) as number[],
  isActivating,
  isActive,
  error,
  setError,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
        <ChainSelect activeChainId={desiredChainId as number} switchChain={switchChain} chainIds={chainIds as number[]} />
      <div style={{ marginBottom: '1rem' }} />
      {isActive ? (
        error ? (
          <button onClick={() => switchChain(desiredChainId as number)}>Try again?</button>
        ) : (
          <button
            onClick={() => {
        ...
        <button
          onClick={() => switchChain(desiredChainId as number)}
